...
class Mocking:
    ...
    def __init__(
        self,
        verifications_api: VerificationsAPI,
        reservations_api: ReservationsAPI,
        billing_cycle_api: BillingCycleAPI,
        sales_api: SalesAPI,
        wake_api: WakeAPI,
    ):
    ...
    def target(
        self,
        behavior: MockBehavior = MockBehavior.Succeeds,
        receive_policy: MockReceivePolicy = MockReceivePolicy.IncomingTextAndCall,
    ) -> str:
    ...
    def reservation(
        ...
        behavior: MockBehavior = MockBehavior.Succeeds,
        receive_policy: MockReceivePolicy = MockReceivePolicy.IncomingTextAndCall,
        reservation_type: ReservationType = ReservationType.NONRENEWABLE,
    ) -> MockObject[Union[RenewableRentalExpanded, NonrenewableRentalExpanded]]:
        line_type = (
            LineReservationType.VERIFICATION
            if reservation_type == ReservationType.VERIFICATION
            else LineReservationType.RENTAL
        )
        mock_id = _construct_mock(
            MockIdentifierType.MockReservation.name,
            behavior.name,
            line_type.name,
            reservation_type.name,
            receive_policy.name,
        )
        return MockObject[Union[RenewableRentalExpanded, NonrenewableRentalExpanded]](
            id=mock_id, _get_function=lambda: self.reservations_api.details(mock_id)
        )
    ...
    def rental(
        ...
        mock_id = _construct_mock(
            MockIdentifierType.MockRental.name,
            behavior.name,
            line_type.name,
            reservation_type.name,
            receive_policy.name,
    ...
    def verification(
        ...
        behavior: MockBehavior = MockBehavior.Succeeds,
        receive_policy: MockReceivePolicy = MockReceivePolicy.IncomingTextAndCall,
    ) -> MockObject[VerificationExpanded]:
        ...
        """
        mock_id = _construct_mock(
            MockIdentifierType.MockVerification.name,
            behavior.name,
            LineReservationType.VERIFICATION.name,
            ReservationType.VERIFICATION.name,
            receive_policy.name,
        )
        return MockObject[VerificationExpanded](
            id=mock_id, _get_function=lambda: self.verifications_api.details(mock_id)
        )
    ...
    def sale(
        ...
        receive_policy: MockReceivePolicy = MockReceivePolicy.IncomingTextAndCall,
        reservation_type: ReservationType = ReservationType.NONRENEWABLE,
    ) -> MockObject[ReservationSaleExpanded]:
        ...
        mock_id = _construct_mock(
            MockIdentifierType.MockSale.name, behavior.name, line_type.name, reservation_type.name, receive_policy.name
        )
    ...
    def backorder_sale(
        ...
        receive_policy: MockReceivePolicy = MockReceivePolicy.IncomingTextAndCall,
        reservation_type: ReservationType = ReservationType.NONRENEWABLE,
    ) -> MockObject[BackOrderReservationExpanded]:
        ...
        mock_id = _construct_mock(
            MockIdentifierType.MockBackorderReservation.name,
            behavior.name,
            line_type.name,
            reservation_type.name,
            receive_policy.name,
        )
        return MockObject[BackOrderReservationExpanded](
            id=mock_id, _get_function=lambda: self.reservations_api.backorder(mock_id)
        )
    ...
    def extension(self, behavior: MockBehavior = MockBehavior.Succeeds) -> MockObject[RentalExtensionRequest]:
        ...
        return MockObject[RentalExtensionRequest](
            id=mock_id,
            _get_function=lambda: RentalExtensionRequest(mock_id, extension_duration=RentalDuration.THIRTY_DAY),
        )
    ...
    def wake_request(
        ...
        mock_id = _construct_mock(
            MockIdentifierType.MockWakeRequest.name,
            behavior.name,
            line_type.name,
            reservation_type.name,
            receive_policy.name,
