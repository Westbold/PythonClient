...
from textverified.generated.generated_enums import (
    ReservationState, RefundAction, VerificationExpanded, VerificationCompact,
    RenewableRentalCompact, RenewableRentalExpanded, NonrenewableRentalCompact,
    NonrenewableRentalExpanded, ReservationType, CancelAction, ReactivationAction, ReportAction, ReuseAction
)
...
def renewable_rental_expanded():
    ...
    return RenewableRentalExpanded(
        ...
        refund=RefundAction(
            can_refund=True,
            refundable_until=datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=2)
        ),
        sale_id="sale_string",
        service_name="test_service",
...
def _find_mock_file(request_path: str, file_list: List[Path]) -> Optional[Dict]:
    ...
    for file_path in file_list:
        ...
        file_path_pattern = "^" + re.sub(r'\{([^}/?&]+)\}', r'[^\/}?&.]*?', str(file_name)) + '$' # file path with 1 group for each path parameter
