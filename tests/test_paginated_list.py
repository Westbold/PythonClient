import pytest
from .fixtures import tv_valid_bearer, mock_http_from_disk
...
list_initial_response = {
    ...
    "links": {
        "next": {
            "method": "GET",
            "href": "/api/pub/v2/list/page2"
        }
    }
}

def test_paginated_list_calls_parse_func(tv_valid_bearer):
    ...
    list_instance = PaginatedList(
        request_json=list_response,
        parse_item=lambda item: item["name"],
        api_context=tv_valid_bearer
    )

    assert list(list_instance) == ["Item 1", "Item 2"]

def test_paginated_list_next_page(tv_valid_bearer, mock_http_from_disk):
...
def test_paginated_list_item_access(tv_valid_bearer, mock_http_from_disk):
    list_instance = PaginatedList(
        request_json=list_initial_response,
        parse_item=lambda item: item["name"],
        api_context=tv_valid_bearer
...
def test_paginated_list_lazy(tv_valid_bearer, mock_http_from_disk):
    list_instance = PaginatedList(
        request_json=list_initial_response,
        parse_item=lambda item: item["name"],
        api_context=tv_valid_bearer
...
def test_paginated_list_get_all(tv_valid_bearer, mock_http_from_disk):
    list_instance = PaginatedList(
        request_json=list_initial_response,
        parse_item=lambda item: item["name"],
        api_context=tv_valid_bearer
